...
Every significant agent output (code, docs, analyses, plans) → write to disk + associate with graph revision via reflect.
...
3. Store:
   - **New item**: `kumiho_memory_store(artifact_location=<path>, memory_item_kind=<kind>, ...)` — or capture via reflect
