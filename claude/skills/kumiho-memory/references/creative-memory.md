...
5. **Action record + edges** — capture via `kumiho_memory_reflect(response=<what was produced>, captures=[{type:"summary", title:"Created <name> on <date>", content:<what was produced + path>, tags:["creative-output","<kind>"]}], source_krefs=[<rev>])`
