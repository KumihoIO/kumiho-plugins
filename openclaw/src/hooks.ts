...
const MEMORY_AGENT_INSTRUCTIONS = [
  ...
  "- REFLECT (after responding): after a substantive exchange, call `memory_reflect` with a one-line summary of your response and structured captures (decisions, preferences, facts, corrections). Pass source_krefs from engage or from the auto-recalled memories so provenance edges link the capture to what informed it. Use absolute dates in titles (\"on Mar 8\", not \"today\"). Skip captures for trivial exchanges. Give every capture a `spaceHint` — the slug below the project (`decisions`, `facts`, `preferences`, never `CognitiveMemory/decisions`); without one the capture lands at the project root, where automatic revision stacking searches that whole bucket and can fold it onto an unrelated memory.",
  "",
