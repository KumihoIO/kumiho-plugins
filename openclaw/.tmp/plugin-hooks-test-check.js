...
function buildCandidates() {
  ...
  return [
    ...
    {
      python: join2(home, ".kumiho", "venv", BIN, `python${EXT}`),
      absolute: true
    },
...
var IS_WIN, BIN, EXT, _cached;
var init_python_setup = __esm({
  "src/python-setup.ts"() {
    "use strict";
    IS_WIN = platform() === "win32";
    BIN = IS_WIN ? "Scripts" : "bin";
    EXT = IS_WIN ? ".exe" : "";
    _cached = null;
  }
});
...
var init_mcp_bridge = __esm({
  "src/mcp-bridge.ts"() {
    ...
    McpBridge = class extends EventEmitter {
      proc = null;
      reader = null;
      nextId = 1;
...
// src/client.ts
function createTransport(config3, logger) {
  if (config3.mode === "local") {
    return new McpTransport(config3, logger);
  }
  return new HttpTransport(config3);
}
var KumihoApiError, HttpTransport, McpTransport, KumihoClient;
var init_client = __esm({
  "src/client.ts"() {
    "use strict";
    init_mcp_bridge();
    KumihoApiError = class extends Error {
    ...
    HttpTransport = class {
      ...
      timeout;
      constructor(config3) {
        this.baseUrl = config3.endpoint.replace(/\/+$/, "");
    ...
    McpTransport = class {
      ...
      constructor(config3, logger) {
        this.bridge = new McpBridge({
          pythonPath: config3.local.pythonPath,
          command: config3.local.command,
          args: config3.local.args,
          env: config3.local.env,
          cwd: config3.local.cwd,
          timeout: config3.local.timeout,
          logger
        });
      }
    ...
    KumihoClient = class {
      ...
      async getRevisions(krefs, includeArtifact = false) {
        return Promise.all(krefs.map((k2) => this.getRevision(k2, includeArtifact)));
      }
      // -----------------------------------------------------------------------
      // Memory management
      // -----------------------------------------------------------------------
...
var init_privacy = __esm({
  "src/privacy.ts"() {
    ...
    PIIRedactor = class {
      ...
      nextPlaceholder(type3) {
        const count = (this.counters.get(type3) ?? 0) + 1;
        this.counters.set(type3, count);
        return `${type3.toUpperCase()}_${String(count).padStart(3, "0")}`;
      }
      /**
...
var DEFAULT_ARTIFACT_ROOT, ArtifactManager;
var init_artifacts = __esm({
  "src/artifacts.ts"() {
    "use strict";
    DEFAULT_ARTIFACT_ROOT = join3(homedir2(), ".kumiho", "artifacts");
    ArtifactManager = class {
      ...
      async saveConversation(project, sessionId, messages, summary) {
        ...
        const dir = join3(this.root, project, space);
        await ensureDir(dir);
      ...
      async saveExecutionLog(project, task, params) {
        const dir = join3(this.root, project, "executions");
        await ensureDir(dir);
      ...
      getProjectDir(project) {
        return join3(this.root, project);
      }
    };
  }
});
...
async function userHash(canonicalId) {
  ...
  const hex3 = Array.from(new Uint8Array(digest)).map((b2) => b2.toString(16).padStart(2, "0")).join("");
  return hex3.slice(0, 10);
}
function utcDate() {
...
async function recordUserTurn(client2, config3, state, userMessage, channel) {
  if (!state.sessionId) {
    state.sessionId = await generateSessionId(config3.userId);
  }
...
async function prefetchMemories(client2, config3, query) {
  const memories = await client2.memoryRetrieve({ query, limit: config3.topK });
  const relevant = memories.filter(
    (m2) => m2.score == null || m2.score >= config3.searchThreshold
  );
...
function buildRecallQuery(userMessage, state) {
  ...
  for (const part of parts) {
    for (const word of part.split(/\s+/)) {
      const w2 = word.toLowerCase().replace(/[^\w]/g, "");
      if (w2.length > 2 && !seen.has(w2)) {
        seen.add(w2);
        tokens.push(word);
      }
    }
...
async function autoRecall(client2, config3, state, userMessage, channel) {
  if (!state.sessionId) {
    state.sessionId = await generateSessionId(config3.userId);
  }
  ...
  state.messageCount++;
  const channelMeta = channel ? buildChannelMetadata(channel) : {};
  const spacePaths = channel && channel.channelType !== "personal_dm" ? [getMemorySpace(channel, config3.project)] : void 0;
...
async function autoCapture(client2, config3, state, redactor2, artifacts2, assistantResponse, channel) {
  ...
  if (state.messageCount >= config3.consolidationThreshold) {
...
async function generateConsolidationSummary(config3, messages) {
  if (!config3.localSummarization) {
    return { summary: null, reason: "local summarization is disabled" };
  }
  const explicitProvider = config3.consolidationModel.provider || config3.llm.provider;
  const explicitApiKey = config3.consolidationModel.apiKey || config3.llm.apiKey;
  if (explicitProvider && !explicitApiKey && config3.hostLlmProvider && explicitProvider !== config3.hostLlmProvider) {
    return {
      summary: null,
      reason: `configured consolidation provider "${explicitProvider}" does not match the available host provider "${config3.hostLlmProvider}"`
    };
  }
  const provider = explicitProvider || config3.hostLlmProvider;
  const apiKey = explicitApiKey || config3.hostLlmApiKey;
  const model = config3.consolidationModel.model || config3.llm.model;
  if (!provider || !apiKey) {
    return {
      summary: null,
  ...
  const transcript = messages.filter((m2) => m2.role === "user" || m2.role === "assistant").map((m2) => `${m2.role === "user" ? "User" : "Assistant"}: ${m2.content}`).join("\n\n").slice(0, 8e3);
...
async function consolidateSession(client2, config3, state, redactor2, artifacts2, channel) {
  ...
  try {
    ...
    const userText = working.messages.filter((m2) => m2.role === "user").map((m2) => m2.content).join("\n");
    const assistantText = working.messages.filter((m2) => m2.role === "assistant").map((m2) => m2.content).join("\n");
    const { summary: llmSummary, reason: summaryFallbackReason } = await generateConsolidationSummary(config3, working.messages);
    let summaryText = llmSummary ?? `Consolidated ${working.message_count} messages from session ${state.sessionId}`;
    if (!llmSummary && config3.localSummarization) {
      console.warn(
        `[kumiho] consolidation for ${state.sessionId} fell back to static summary: ${summaryFallbackReason ?? "unknown reason"}`
      );
    }
    if (config3.piiRedaction) {
      const redacted = redactor2.redact(summaryText);
      summaryText = redactor2.anonymizeSummary(redacted.text);
    }
    const spaceHint = channel ? getMemorySpace(channel, config3.project).replace(`${config3.project}/`, "") : "personal";
    await client2.memoryStore({
      type: "summary",
      title: `Session consolidation: ${state.sessionId}`,
      summary: summaryText,
      userText: config3.privacy.uploadSummariesOnly ? summaryText : userText,
      assistantText: config3.privacy.uploadSummariesOnly ? "" : assistantText,
    ...
    state.sessionId = await generateSessionId(config3.userId, "personal", true);
...
async function creativeRecallHandler(ctx, params) {
  ...
  try {
    ...
    if (query) {
      ...
      items = (raw.results ?? []).map((r2) => ({ ...r2.item, score: r2.score }));
    } else {
      const raw = await ctx.client.callTool(
        "kumiho_search_items",
        {
  ...
  if ((params.scope === "session" || params.scope === "all") && ctx.currentSessionId) {
    ...
    if (working.messages.length > 0) {
      const sessionSection = working.messages.filter(
        (m2) => m2.content.toLowerCase().includes(params.query.toLowerCase())
      ).map((m2) => `[session] ${m2.role}: ${m2.content}`).join("\n");
      if (sessionSection) {
...
async function handleMemoryList(ctx, params) {
  ...
  if ((params.scope === "session" || params.scope === "all" || !params.scope) && ctx.currentSessionId) {
    const working = await ctx.client.chatGet(ctx.currentSessionId, limit);
    if (working.messages.length > 0) {
      const msgs = working.messages.map((m2) => `- **${m2.role}** (${m2.timestamp}): ${m2.content.slice(0, 100)}`).join("\n");
      sections.push(
...
async function handleMemoryForget(ctx, params) {
  ...
  if (params.query) {
    ...
    for (const entry of results) {
      ...
      deprecated2.push(entry.kref);
    }
    return `Deprecated ${deprecated2.length} memories:
${deprecated2.join("\n")}`;
  }
...
async function handleMemoryConsolidate(ctx, params) {
  ...
  const conversationText = working.messages.map((m2) => `${m2.role}: ${m2.content}`).join("\n");
  const result = await ctx.client.memoryStore({
    type: "summary",
...
var TOOL_SCHEMAS, TOOL_HANDLERS;
var init_tools = __esm({
  "src/tools.ts"() {
    ...
    TOOL_HANDLERS = {
      memory_search: (ctx, p3) => handleMemorySearch(ctx, p3),
      memory_store: (ctx, p3) => handleMemoryStore(ctx, p3),
      memory_get: (ctx, p3) => handleMemoryGet(ctx, p3),
      memory_list: (ctx, p3) => handleMemoryList(ctx, p3),
      memory_forget: (ctx, p3) => handleMemoryForget(ctx, p3),
      memory_consolidate: (ctx, p3) => handleMemoryConsolidate(ctx, p3),
      memory_dream: (ctx) => handleMemoryDream(ctx),
      creative_capture: (ctx, p3) => creativeCaptureHandler(ctx, p3),
      creative_job_status: (ctx, p3) => creativeJobStatusHandler(ctx, p3),
      creative_recall: (ctx, p3) => creativeRecallHandler(ctx, p3)
    };
  }
});

// src/identity.ts
async function ensureUserIdentity(client2, config3, senderId, identity) {
  ...
  try {
    const existing = await client2.memoryRetrieve({
      ...
      limit: 1,
      spacePaths: [`${config3.project}/users`]
    });
    const hasProfile = existing.some(
      (m2) => m2.metadata?.userId === senderId
    );
...
__export(index_exports, {
  ...
  KumihoApiError: () => KumihoApiError,
  KumihoClient: () => KumihoClient,
  McpBridge: () => McpBridge,
  McpBridgeError: () => McpBridgeError,
  PIIRedactor: () => PIIRedactor,
  TOOL_HANDLERS: () => TOOL_HANDLERS,
  TOOL_SCHEMAS: () => TOOL_SCHEMAS,
  createKumihoMemory: () => createKumihoMemory,
  createTransport: () => createTransport,
  default: () => index_default,
  ensureUserIdentity: () => ensureUserIdentity,
  generateSessionId: () => generateSessionId,
  getMemorySpace: () => getMemorySpace,
  inferChannelType: () => inferChannelType
});
...
function loadPreferences() {
  try {
    const path = join4(homedir3(), ".kumiho", "preferences.json");
    if (existsSync2(path)) {
      const raw = JSON.parse(readFileSync(path, "utf8"));
      for (const section of ["dreamState", "consolidation"]) {
        const s2 = raw[section];
        if (s2?.["model"] && typeof s2["model"] === "object") {
          const m2 = s2["model"];
          delete m2["apiKey"];
          delete m2["api_key"];
        }
...
function loadAuthToken() {
  try {
    const path = join4(homedir3(), ".kumiho", "kumiho_authentication.json");
    if (!existsSync2(path)) return "";
    const auth = JSON.parse(readFileSync(path, "utf8"));
    if (auth.api_token) {
      const now3 = Math.floor(Date.now() / 1e3);
      if (!auth.api_token_expires_at || auth.api_token_expires_at > now3) {
        return auth.api_token;
      }
    }
    if (auth.id_token) {
      const now3 = Math.floor(Date.now() / 1e3);
      if (!auth.expires_at || auth.expires_at > now3) {
...
function extractCredentialString(value, depth = 0) {
  ...
  for (const nested of Object.values(record)) {
    const found2 = extractCredentialString(nested, depth + 1);
...
function loadOpenClawAuthProfile(preferredProvider) {
  try {
    const path = join4(homedir3(), ".openclaw", "agents", "main", "agent", "auth-profiles.json");
    ...
    if (preferredProvider) {
      ...
      const preferred = candidates.filter((entry) => entry.provider === preferredProvider).sort((a3, b2) => b2.lastUsed - a3.lastUsed || a3.errorCount - b2.errorCount)[0];
      if (preferred) {
        return { provider: preferred.provider, apiKey: preferred.apiKey };
      }
    }
    const rankedLastGood = Object.values(lastGood).map((profileKey) => candidates.find((entry) => entry.profileKey === profileKey)).filter((entry) => !!entry).sort((a3, b2) => b2.lastUsed - a3.lastUsed);
    if (rankedLastGood[0]) {
      return {
    ...
    const best = [...candidates].sort((a3, b2) => a3.errorCount - b2.errorCount || b2.lastUsed - a3.lastUsed)[0];
    if (best) {
...
function resolveConfig(raw, inheritedHostLlm) {
  ...
  return {
    ...
    artifactDir: raw.artifactDir || process.env.KUMIHO_MEMORY_ARTIFACT_ROOT || join4(homedir3(), ".kumiho", "artifacts"),
...
function msUntilNextCron(cron) {
  ...
  if (everyHour) {
    ...
    const currentHour = now3.getHours();
    const nextHour = Math.ceil((currentHour + 1) / interval) * interval;
    const next2 = new Date(now3);
    ...
    const ms = next2.getTime() - now3.getTime();
  ...
  if (weekdayPart !== "*") {
    ...
    const daysUntil = (targetDay - now3.getDay() + 7) % 7 || 7;
  ...
  if (next <= now3) next.setDate(next.getDate() + 1);
  return next.getTime() - now3.getTime();
}
...
function ensureInitialized() {
  if (!transport || !client || !config2 || !redactor || !artifacts || !hookState) {
    throw new Error("Kumiho plugin not initialized. Check your configuration.");
  }
  return { transport, client, config: config2, redactor, artifacts, hookState };
}
...
var _AUTH_PATH, transport, client, config2, redactor, artifacts, hookState, startPromise, runtimeStatusLogged, idleTimer, dreamStateTimer, index_default;
var init_index = __esm({
  "src/index.ts"() {
    ...
    init_identity();
    init_tools();
    _AUTH_PATH = join4(homedir3(), ".kumiho", "kumiho_authentication.json");
    transport = null;
    client = null;
    config2 = null;
    redactor = null;
    ...
    index_default = {
      ...
      register(api) {
        ...
        try {
          config2 = resolveConfig(rawConfig, inheritedHostLlm);
        } catch (err) {
          api.logger.error(`Kumiho: ${err.message}`);
          return;
        }
        const refreshResolvedHostLlm = (target = config2) => {
          if (!target) return;
          const latest = resolveInheritedHostLlm();
          target.hostLlmProvider = latest?.provider ?? "";
          target.hostLlmApiKey = latest?.apiKey ?? "";
        };
        if (config2.localSummarization && !config2.consolidationModel.apiKey && !config2.llm.apiKey && !config2.hostLlmApiKey) {
          api.logger.warn(
        ...
        const dreamProviderHint = config2.dreamStateModel.provider || config2.llm.provider;
        if (dreamProviderHint && !config2.dreamStateModel.apiKey && !config2.llm.apiKey && config2.hostLlmProvider && dreamProviderHint !== config2.hostLlmProvider) {
          api.logger.warn(
            `Kumiho: dreamStateModel/llm provider "${dreamProviderHint}" does not match the available host credential provider "${config2.hostLlmProvider}". Dream State will stay disabled unless OpenClaw exposes a matching host provider or you configure a matching model credential explicitly.`
          );
        }
        if (config2.mode === "local") {
          const dreamModelConfig = resolveDreamStateModelConfig(config2);
          const extra = {};
          ...
          if (Object.keys(extra).length) {
            config2.local.env = { ...config2.local.env, ...extra };
          }
        }
        transport = createTransport(config2, api.logger);
        client = new KumihoClient(transport, config2.project);
        redactor = new PIIRedactor();
        artifacts = new ArtifactManager(config2.artifactDir);
        hookState = createHookState();
        api.logger.info(
          `Kumiho memory initialized (mode: ${config2.mode}, project: ${config2.project}, autoRecall: ${config2.autoRecall}, autoCapture: ${config2.autoCapture})`
        );
        const toolCtx = {
          client,
          config: config2,
...
Processing in background. Use creative_recall to see it once done.`
        ...
        api.on("before_prompt_build", async (event, _ctx) => {
          clearIdleTimer();
          if (!config2?.autoRecall) return;
          try {
            const state = await ensureRuntimeStarted(api.logger);
            const messages = event.messages ?? [];
            const lastUserMsg = [...messages].reverse().find((m2) => m2.role === "user");
            const raw = lastUserMsg?.content;
            const message = typeof raw === "string" ? raw : Array.isArray(raw) ? raw.filter((b2) => b2.type === "text").map((b2) => b2.text).join("\n") : state.hookState.lastUserMessage ?? "";
            let recallResult;
            if (state.hookState.prefetchedRecall) {
              ...
              void prefetchMemories(state.client, state.config, message).then((r2) => {
                state.hookState.prefetchedRecall = r2;
              }).catch(() => {
              });
            } else {
              recallResult = await Promise.race([
                ...
                new Promise(
                  (resolve4) => setTimeout(() => resolve4({ contextInjection: "" }), 1500)
                )
              ]);
            }
        ...
        api.on("agent_end", async (event, _ctx) => {
          if (!config2?.autoCapture) return;
          try {
            const state = await ensureRuntimeStarted(api.logger);
            refreshResolvedHostLlm(state.config);
            const messages = event.messages ?? [];
            const lastAssistantMsg = [...messages].reverse().find((m2) => m2.role === "assistant");
            ...
            if (typeof raw === "string") {
              ...
              const blocks = raw;
              const textParts = blocks.filter((b2) => b2.type === "text").map((b2) => b2.text ?? "");
              const toolParts = blocks.filter((b2) => b2.type === "tool_use").map((b2) => `[tool: ${b2.name}]`);
            ...
            if (config2?.autoRecall && state.hookState.lastUserMessage) {
              const queryForNext = state.hookState.lastUserMessage;
              void prefetchMemories(state.client, state.config, queryForNext).then((r2) => {
                state.hookState.prefetchedRecall = r2;
              }).catch(() => {
              });
            }
            const idleMs = (config2?.idleConsolidationTimeout ?? 0) * 1e3;
...
function printIteratorEntries(iterator, config3, indentation, depth, refs, printer2, separator = ": ") {
  ...
  if (!current.done) {
    ...
    while (!current.done) {
      ...
      if (width++ === config3.maxWidth) {
        result += "\u2026";
        break;
      }
      const name = printer2(current.value[0], config3, indentationNext, depth, refs);
      const value = printer2(current.value[1], config3, indentationNext, depth, refs);
...
function printIteratorValues(iterator, config3, indentation, depth, refs, printer2) {
  ...
  if (!current.done) {
    ...
    while (!current.done) {
      ...
      result += printer2(current.value, config3, indentationNext, depth, refs);
      current = iterator.next();
      if (!current.done) {
        result += `,${config3.spacingInner}`;
      } else if (!config3.min) {
...
function printListItems(list, config3, indentation, depth, refs, printer2) {
  ...
  if (length > 0) {
    ...
    for (let i = 0; i < length; i++) {
      result += indentationNext;
      if (i === config3.maxWidth) {
        result += "\u2026";
        break;
      }
      if (isDataView(list) || i in list) {
        result += printer2(isDataView(list) ? list.getInt8(i) : list[i], config3, indentationNext, depth, refs);
      }
      if (i < length - 1) {
        result += `,${config3.spacingInner}`;
      } else if (!config3.min) {
...
var serialize$5 = (val, config3, indentation, depth, refs, printer2) => {
  ...
  if (stringedValue === "ArrayContaining" || stringedValue === "ArrayNotContaining") {
    ...
    return `${stringedValue + SPACE$2}[${printListItems(val.sample, config3, indentation, depth, refs, printer2)}]`;
  }
  if (stringedValue === "ObjectContaining" || stringedValue === "ObjectNotContaining") {
    if (++depth > config3.maxDepth) {
      return `[${stringedValue}]`;
    }
    return `${stringedValue + SPACE$2}{${printObjectProperties(val.sample, config3, indentation, depth, refs, printer2)}}`;
  }
...
var serialize$4 = (collection, config3, indentation, depth, refs, printer2) => {
  ...
  return (config3.min ? "" : name + SPACE$1) + (OBJECT_NAMES.has(name) ? `{${printObjectProperties(isNamedNodeMap(collection) ? [...collection].reduce((props, attribute) => {
    props[attribute.name] = attribute.value;
    return props;
  }, {}) : { ...collection }, config3, indentation, depth, refs, printer2)}}` : `[${printListItems([...collection], config3, indentation, depth, refs, printer2)}]`);
};
var plugin$4 = {
...
function printElement(type3, printedProps, printedChildren, config3, indentation) {
...
function printElementAsLeaf(type3, config3) {
  const tagColor = config3.colors.tag;
...
var serialize$3 = (node, config3, indentation, depth, refs, printer2) => {
  ...
  return printElement(type3, printProps(nodeIsFragment(node) ? [] : Array.from(node.attributes, (attr) => attr.name).sort(), nodeIsFragment(node) ? {} : [...node.attributes].reduce((props, attribute) => {
    props[attribute.name] = attribute.value;
    return props;
  }, {}), config3, indentation + config3.indent, depth, refs, printer2), printChildren(Array.prototype.slice.call(node.childNodes || node.children), config3, indentation + config3.indent, depth, refs, printer2), config3, indentation);
};
var plugin$3 = {
...
var SPACE = " ";
var LAZY = "\u2026";
function printImmutableEntries(val, config3, indentation, depth, refs, printer2, type3) {
  return ++depth > config3.maxDepth ? printAsLeaf(getImmutableName(type3)) : `${getImmutableName(type3) + SPACE}{${printIteratorEntries(val.entries(), config3, indentation, depth, refs, printer2)}}`;
}
...
function printImmutableSeq(val, config3, indentation, depth, refs, printer2) {
  ...
  if (val[IS_KEYED_SENTINEL]) {
    return `${name + SPACE}{${val._iter || val._object ? printIteratorEntries(val.entries(), config3, indentation, depth, refs, printer2) : LAZY}}`;
  }
  return `${name + SPACE}[${val._iter || val._array || val._collection || val._iterable ? printIteratorValues(val.values(), config3, indentation, depth, refs, printer2) : LAZY}]`;
}
function printImmutableValues(val, config3, indentation, depth, refs, printer2, type3) {
  return ++depth > config3.maxDepth ? printAsLeaf(getImmutableName(type3)) : `${getImmutableName(type3) + SPACE}[${printIteratorValues(val.values(), config3, indentation, depth, refs, printer2)}]`;
}
var serialize$2 = (val, config3, indentation, depth, refs, printer2) => {
  if (val[IS_MAP_SENTINEL]) {
    return printImmutableEntries(val, config3, indentation, depth, refs, printer2, val[IS_ORDERED_SENTINEL] ? "OrderedMap" : "Map");
  }
  if (val[IS_LIST_SENTINEL]) {
    return printImmutableValues(val, config3, indentation, depth, refs, printer2, "List");
  }
  if (val[IS_SET_SENTINEL]) {
    return printImmutableValues(val, config3, indentation, depth, refs, printer2, val[IS_ORDERED_SENTINEL] ? "OrderedSet" : "Set");
  }
  if (val[IS_STACK_SENTINEL]) {
    return printImmutableValues(val, config3, indentation, depth, refs, printer2, "Stack");
  }
...
var serialize = (object2, config3, indentation, depth, refs, printer2) => ++depth > config3.maxDepth ? printElementAsLeaf(object2.type, config3) : printElement(object2.type, object2.props ? printProps(getPropKeys(object2), object2.props, config3, indentation + config3.indent, depth, refs, printer2) : "", object2.children ? printChildren(object2.children, config3, indentation + config3.indent, depth, refs, printer2) : "", config3, indentation);
...
function printComplexValue(val, config3, indentation, depth, refs, hasCalledToJSON) {
  ...
  if (toStringed === "[object Arguments]") {
    return hitMaxDepth ? "[Arguments]" : `${min ? "" : "Arguments "}[${printListItems(val, config3, indentation, depth, refs, printer)}]`;
  }
  if (isToStringedArrayType(toStringed)) {
    return hitMaxDepth ? `[${val.constructor.name}]` : `${min ? "" : !config3.printBasicPrototype && val.constructor.name === "Array" ? "" : `${val.constructor.name} `}[${printListItems(val, config3, indentation, depth, refs, printer)}]`;
  }
  if (toStringed === "[object Map]") {
    return hitMaxDepth ? "[Map]" : `Map {${printIteratorEntries(val.entries(), config3, indentation, depth, refs, printer, " => ")}}`;
  }
  if (toStringed === "[object Set]") {
    return hitMaxDepth ? "[Set]" : `Set {${printIteratorValues(val.values(), config3, indentation, depth, refs, printer)}}`;
  }
  return hitMaxDepth || isWindow(val) ? `[${getConstructorName(val)}]` : `${min ? "" : !config3.printBasicPrototype && getConstructorName(val) === "Object" ? "" : `${getConstructorName(val)} `}{${printObjectProperties(val, config3, indentation, depth, refs, printer)}}`;
}
var ErrorPlugin = {
...
function printPlugin(plugin3, val, config3, indentation, depth, refs) {
  let printed;
  try {
    printed = isNewPlugin(plugin3) ? plugin3.serialize(val, config3, indentation, depth, refs, printer) : plugin3.print(val, (valChild) => printer(valChild, config3, indentation, depth, refs), (str) => {
      const indentationNext = indentation + config3.indent;
...
function printer(val, config3, indentation, depth, refs, hasCalledToJSON) {
  const plugin3 = findPlugin(config3.plugins, val);
  if (plugin3 !== null) {
    return printPlugin(plugin3, val, config3, indentation, depth, refs);
  }
  const basicResult = printBasicValue(val, config3.printFunctionName, config3.escapeRegex, config3.escapeString);
  if (basicResult !== null) {
    return basicResult;
  }
...
var serialize$12 = (val, config3, indentation, depth, refs, printer2) => {
  const name = val.getMockName();
  const nameString = name === "vi.fn()" ? "" : ` ${name}`;
  let callsString = "";
  if (val.mock.calls.length !== 0) {
    const indentationNext = indentation + config3.indent;
    callsString = ` {${config3.spacingOuter}${indentationNext}"calls": ${printer2(val.mock.calls, config3, indentationNext, depth, refs)}${config3.min ? ", " : ","}${config3.spacingOuter}${indentationNext}"results": ${printer2(val.mock.results, config3, indentationNext, depth, refs)}${config3.min ? "" : ","}${config3.spacingOuter}${indentation}}`;
  }
...
To automatically clean-up native timers, use \`shouldClearNativeTimers\`.`
    ...
    function uninstall(clock, config3) {
      let method, i, l2;
      ...
      if (config3.shouldAdvanceTime === true) {
    ...
    function install(config3) {
      if (arguments.length > 1 || config3 instanceof Date || Array.isArray(config3) || typeof config3 === "number") {
        throw new TypeError(
      ...
      config3 = typeof config3 !== "undefined" ? config3 : {};
      config3.shouldAdvanceTime = config3.shouldAdvanceTime || false;
      config3.advanceTimeDelta = config3.advanceTimeDelta || 20;
      config3.shouldClearNativeTimers = config3.shouldClearNativeTimers || false;
      if (config3.target) {
        throw new TypeError(
      ...
      function handleMissingTimer(timer) {
        if (config3.ignoreMissingTimers) {
          return;
        }
      ...
      let i, l2;
      const clock = createClock(config3.now, config3.loopLimit);
      clock.shouldClearNativeTimers = config3.shouldClearNativeTimers;
      clock.uninstall = function() {
        return uninstall(clock, config3);
      };
      clock.abortListenerMap = /* @__PURE__ */ new Map();
      clock.methods = config3.toFake || [];
      ...
      if (clock.methods.includes("performance")) {
        ...
        if (proto) {
          ...
          clock.performance.timeOrigin = getEpoch(config3.now);
        } else if ((config3.toFake || []).includes("performance")) {
...
var FakeTimers = class {
  ...
  _userConfig;
  _now = RealDate.now;
  constructor({ global: global3, config: config3 }) {
    this._userConfig = config3;
...
function createVitest() {
  ...
  const utils = {
    useFakeTimers(config3) {
      if (isChildProcess()) {
        if (config3?.toFake?.includes("nextTick") || workerState.config?.fakeTimers?.toFake?.includes("nextTick")) throw new Error('vi.useFakeTimers({ toFake: ["nextTick"] }) is not supported in node:child_process. Use --pool=threads if mocking nextTick is required.');
      }
      if (config3) timers().configure({
    ...
    setConfig(config3) {
      ...
      Object.assign(workerState.config, config3);
    },
    resetConfig() {
...
var mockState = vi.hoisted(() => {
  ...
  const transports = [];
  const createTransport2 = vi.fn(() => {
    const transport2 = new FakeMcpTransport();
...
describe("OpenClaw prompt hooks", () => {
  ...
  it("lazy-starts the MCP bridge when before_prompt_build fires before service start", async () => {
    const { default: plugin3 } = await Promise.resolve().then(() => (init_index(), index_exports));
    const api = makeApi();
    plugin3.register(api);
    globalExpect(mockState.createTransport).toHaveBeenCalledOnce();
    globalExpect(mockState.transports).toHaveLength(1);
    globalExpect(mockState.transports[0].startCalls).toBe(0);
    const beforePromptBuild = api.events.get("before_prompt_build");
    const agentEnd = api.events.get("agent_end");
    globalExpect(beforePromptBuild).toBeTypeOf("function");
    globalExpect(agentEnd).toBeTypeOf("function");
    ...
    globalExpect(mockState.transports[0].startCalls).toBe(1);
    await agentEnd?.(
      {
    ...
    globalExpect(storedMessages.map((message) => message.role)).toEqual(["user", "assistant"]);
    globalExpect(
      api.logger.error.mock.calls.some(
        ([message]) => String(message).includes("Bridge not initialized")
      )
    ).toBe(false);
  });
  it("uses host LLM credentials from the OpenClaw runtime config when available", async () => {
    ...
    plugin3.register(api);
    const [resolvedConfig] = mockState.createTransport.mock.calls[0];
    globalExpect(resolvedConfig.hostLlmProvider).toBe("anthropic");
    globalExpect(resolvedConfig.hostLlmApiKey).toBe("host-anthropic-key");
    globalExpect(api.logger.warn).not.toHaveBeenCalledWith(
      globalExpect.stringContaining("no host LLM credentials were resolved")
    );
  });
  it("parses the current auth-profiles schema under auth.profiles", async () => {
    vi.doMock("node:fs", async () => {
      ...
      return {
        ...actual,
        existsSync: vi.fn((path) => path.endsWith("auth-profiles.json")),
        readFileSync: vi.fn(
          () => JSON.stringify({
            auth: {
              profiles: {
    ...
    const [resolvedConfig] = mockState.createTransport.mock.calls[0];
    globalExpect(resolvedConfig.hostLlmProvider).toBe("anthropic");
    globalExpect(resolvedConfig.hostLlmApiKey).toBe("sk-ant-host-profile");
  });
  it("uses an unexpired openai-codex oauth access token as the host OpenAI credential", async () => {
    ...
    plugin3.register(api);
    const [resolvedConfig] = mockState.createTransport.mock.calls[0];
    globalExpect(resolvedConfig.hostLlmProvider).toBe("openai");
    globalExpect(resolvedConfig.hostLlmApiKey).toBe("openai-oauth-access-token");
  });
  it("warns when dreamStateModel requests a different provider than the only host credential", async () => {
    ...
    plugin3.register(api);
    globalExpect(api.logger.warn).toHaveBeenCalledWith(
      globalExpect.stringContaining('dreamStateModel/llm provider "openai" does not match')
    );
  });
