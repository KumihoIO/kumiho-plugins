...
async function recordUserTurn(client, config2, state, userMessage, channel) {
  if (!state.sessionId) {
...
async function prefetchMemories(client, config2, query) {
  const memories = await client.memoryRetrieve({ query, limit: config2.topK });
  const relevant = memories.filter(
    (m2) => m2.score == null || m2.score >= config2.searchThreshold
  );
...
async function autoRecall(client, config2, state, userMessage, channel) {
  if (!state.sessionId) {
    state.sessionId = await generateSessionId(config2.userId);
  }
...
async function autoCapture(client, config2, state, redactor, artifacts, assistantResponse, channel) {
...
async function consolidateSession(client, config2, state, redactor, artifacts, channel) {
...
var baseConfig = {
  ...
  local: { pythonPath: "python", command: "kumiho-mcp", timeout: 3e4 }
};
function makeMessages(pairs) {
  const msgs = [];
  for (const p3 of pairs) {
    msgs.push({ role: "user", content: p3.user, timestamp: "2026-03-10T00:00:00Z" });
    msgs.push({ role: "assistant", content: p3.assistant, timestamp: "2026-03-10T00:00:01Z" });
  }
  return msgs;
}
function makeClient(messages) {
  return {
    ...
    chatClear: vi.fn().mockResolvedValue(void 0)
  };
}
...
function makeRedactor() {
  return {
    redact: vi.fn().mockImplementation((text) => ({ text, entities: [] })),
    anonymizeSummary: vi.fn().mockImplementation((text) => text),
    reset: vi.fn()
  };
}
describe("consolidateSession", () => {
  ...
  it("returns false when there are no messages", async () => {
    ...
    globalExpect(result).toBe(false);
    globalExpect(client.memoryStore).not.toHaveBeenCalled();
  });
  it("returns true and calls memoryStore when messages exist", async () => {
    ...
    globalExpect(client.memoryStore).toHaveBeenCalledOnce();
    globalExpect(client.chatClear).toHaveBeenCalledWith("test-session-001");
  });
  it("splits messages by role \u2014 userText contains only user messages", async () => {
    ...
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.userText).toContain("Question one");
    globalExpect(call2.userText).toContain("Question two");
    globalExpect(call2.userText).not.toContain("Answer one");
    globalExpect(call2.userText).not.toContain("Answer two");
  });
  it("splits messages by role \u2014 assistantText contains only assistant messages", async () => {
    ...
    await consolidateSession(client, baseConfig, state, makeRedactor(), makeArtifacts());
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.assistantText).toContain("Answer one");
    globalExpect(call2.assistantText).toContain("Answer two");
    globalExpect(call2.assistantText).not.toContain("Question one");
    globalExpect(call2.assistantText).not.toContain("Question two");
  });
  it("sends summary text for both fields when uploadSummariesOnly is true", async () => {
    ...
    const client = makeClient(messages);
    await consolidateSession(client, config2, state, makeRedactor(), makeArtifacts());
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.assistantText).toBe("");
    globalExpect(call2.userText).not.toContain("Secret question");
    globalExpect(call2.userText).not.toContain("Secret answer");
    globalExpect(call2.userText).toMatch(/Consolidated/i);
  });
  it("sends raw text when uploadSummariesOnly is false", async () => {
    ...
    await consolidateSession(client, config2, state, makeRedactor(), makeArtifacts());
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.userText).toContain("Raw user text");
    globalExpect(call2.assistantText).toContain("Raw assistant text");
  });
  it("resets messageCount and creates a new sessionId after consolidation", async () => {
    ...
    const client = makeClient(messages);
    await consolidateSession(client, baseConfig, state, makeRedactor(), makeArtifacts());
    globalExpect(state.messageCount).toBe(0);
    globalExpect(state.sessionId).not.toBe("test-session-001");
    globalExpect(typeof state.sessionId).toBe("string");
  });
  it("stores summary with required tags", async () => {
    ...
    await consolidateSession(client, baseConfig, state, makeRedactor(), makeArtifacts());
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.tags).toContain("consolidated");
    globalExpect(call2.tags).toContain("summary");
  });
  it("stores the artifact location in memoryStore metadata", async () => {
    ...
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.artifactLocation).toBe("/tmp/artifact.json");
  });
...
describe("autoRecall \u2014 spacePaths scoping", () => {
  function makeRecallClient(memoryRetrieveFn = vi.fn().mockResolvedValue([])) {
    return {
      chatAdd: vi.fn().mockResolvedValue(void 0),
  ...
  it("passes no spacePaths for personal_dm \u2014 searches whole project", async () => {
    ...
    const channel = { channelType: "personal_dm", channelId: "tg:123" };
    await autoRecall(client, recallConfig, state, "what did we work on?", channel);
    globalExpect(memoryRetrieve).toHaveBeenCalledOnce();
    const [params] = memoryRetrieve.mock.calls[0];
    globalExpect(params.spacePaths).toBeUndefined();
  });
  it("restricts spacePaths for team_channel \u2014 isolates team space", async () => {
    const memoryRetrieve = vi.fn().mockResolvedValue([]);
    const client = makeRecallClient(memoryRetrieve);
    const state = createHookState();
    const channel = { channelType: "team_channel", channelId: "slack:C123", teamSlug: "eng" };
    await autoRecall(client, recallConfig, state, "deploy status?", channel);
    const [params] = memoryRetrieve.mock.calls[0];
    globalExpect(params.spacePaths).toBeDefined();
    globalExpect(params.spacePaths[0]).toContain("eng");
  });
  it("restricts spacePaths for group_dm \u2014 isolates group space", async () => {
    const memoryRetrieve = vi.fn().mockResolvedValue([]);
    const client = makeRecallClient(memoryRetrieve);
    const state = createHookState();
    const channel = { channelType: "group_dm", channelId: "tg:group:456", groupId: "456" };
    await autoRecall(client, recallConfig, state, "hey", channel);
    const [params] = memoryRetrieve.mock.calls[0];
    globalExpect(params.spacePaths).toBeDefined();
    globalExpect(params.spacePaths[0]).toContain("456");
  });
...
describe("buildRecallQuery", () => {
  it("returns the message unchanged when it is long enough and has no prior context", () => {
    ...
    globalExpect(result).toContain("LoCoMo");
    globalExpect(result).toContain("benchmark");
  });
  ...
  it("does NOT append previous message when current message is long enough", () => {
    ...
    globalExpect(result).not.toContain("dogs");
    globalExpect(result).not.toContain("weather");
  });
  it("appends first 20 words of last assistant response", () => {
    ...
    globalExpect(result).toContain("LoCoMo-Plus");
    globalExpect(result).toContain("benchmark");
  });
  it("deduplicates words across message and context", () => {
    ...
    const locomo = words.filter((w2) => w2.toLowerCase() === "locomo");
    globalExpect(locomo.length).toBe(1);
  });
  it("caps output at 200 characters", () => {
    ...
    globalExpect(result.length).toBeLessThanOrEqual(200);
  });
  it("autoRecall uses enriched query \u2014 short message gets prior context appended", async () => {
    ...
    await autoRecall(client, { ...baseConfig, topK: 5, searchThreshold: 0 }, state, "yeah?");
    const [params] = memoryRetrieve.mock.calls[0];
    globalExpect(params.query).toContain("locomo");
  });
});
describe("recordUserTurn", () => {
  it("stores user message via chatAdd with correct role and content", async () => {
    const chatAdd = vi.fn().mockResolvedValue(void 0);
    const client = { chatAdd };
    const state = createHookState();
    state.sessionId = "session-rut-001";
    await recordUserTurn(client, baseConfig, state, "hello world");
    globalExpect(chatAdd).toHaveBeenCalledOnce();
    const [sid, role, content] = chatAdd.mock.calls[0];
    globalExpect(sid).toBe("session-rut-001");
    globalExpect(role).toBe("user");
    globalExpect(content).toBe("hello world");
  });
  it("sets state.lastUserMessage", async () => {
    const client = { chatAdd: vi.fn().mockResolvedValue(void 0) };
    const state = createHookState();
    state.sessionId = "session-rut-002";
    await recordUserTurn(client, baseConfig, state, "test message");
    globalExpect(state.lastUserMessage).toBe("test message");
  });
  it("increments state.messageCount", async () => {
    ...
    globalExpect(state.messageCount).toBe(4);
  });
  it("initialises sessionId when null", async () => {
    const client = { chatAdd: vi.fn().mockResolvedValue(void 0) };
    const state = createHookState();
    globalExpect(state.sessionId).toBeNull();
    await recordUserTurn(client, baseConfig, state, "hello");
    globalExpect(state.sessionId).not.toBeNull();
    globalExpect(typeof state.sessionId).toBe("string");
  });
  it("includes channel metadata in chatAdd call when channel provided", async () => {
    ...
    const channel = { platform: "telegram", channelType: "personal_dm" };
    await recordUserTurn(client, baseConfig, state, "msg", channel);
    const [, , , meta] = chatAdd.mock.calls[0];
    globalExpect(meta).toHaveProperty("channel");
    globalExpect(meta.channel.platform).toBe("telegram");
  });
  it("propagates chatAdd rejection to caller", async () => {
    ...
    await globalExpect(recordUserTurn(client, baseConfig, state, "msg")).rejects.toThrow("Redis down");
  });
});
describe("consolidateSession \u2014 LLM summarization", () => {
  ...
  it("uses static fallback when localSummarization is false", async () => {
    ...
    await consolidateSession(client, baseConfig, state, makeRedactor(), makeArtifacts());
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.summary).toMatch(/Consolidated/i);
  });
  it("uses static fallback when no LLM provider is configured", async () => {
    ...
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.summary).toMatch(/Consolidated/i);
    globalExpect(warn).toHaveBeenCalledWith(
      globalExpect.stringContaining("no LLM provider/API key was resolved")
    );
  });
  it("calls Anthropic API and uses LLM response as summary", async () => {
    ...
    await consolidateSession(client, config2, state, makeRedactor(), makeArtifacts());
    globalExpect(mockFetch).toHaveBeenCalledOnce();
    const [url, init] = mockFetch.mock.calls[0];
    globalExpect(url).toBe("https://api.anthropic.com/v1/messages");
    const headers = init.headers;
    globalExpect(headers["x-api-key"]).toBe("sk-ant-test-key");
    globalExpect(headers["anthropic-version"]).toBeDefined();
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.summary).toBe(llmSummary);
  });
  it("calls OpenAI API and uses LLM response as summary", async () => {
    ...
    globalExpect(mockFetch).toHaveBeenCalledOnce();
    const [url, init] = mockFetch.mock.calls[0];
    globalExpect(url).toBe("https://api.openai.com/v1/chat/completions");
    const headers = init.headers;
    globalExpect(headers["Authorization"]).toBe("Bearer sk-openai-test-key");
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.summary).toBe(llmSummary);
  });
  it("calls OpenAI Responses API for codex models", async () => {
    ...
    globalExpect(mockFetch).toHaveBeenCalledOnce();
    const [url, init] = mockFetch.mock.calls[0];
    globalExpect(url).toBe("https://api.openai.com/v1/responses");
    const headers = init.headers;
    globalExpect(headers["Authorization"]).toBe("Bearer oauth-access-token");
    const body = JSON.parse(init.body);
    globalExpect(body.model).toBe("gpt-5-codex");
    globalExpect(body.input).toContain("Summarize the following conversation");
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.summary).toBe(llmSummary);
  });
  it("falls back to static summary when fetch rejects", async () => {
    ...
    const result = await consolidateSession(client, config2, state, makeRedactor(), makeArtifacts());
    globalExpect(result).toBe(true);
    const call2 = vi.mocked(client.memoryStore).mock.calls[0][0];
    globalExpect(call2.summary).toMatch(/Consolidated/i);
    globalExpect(warn).toHaveBeenCalledWith(
      globalExpect.stringContaining("network error")
    );
  });
  it("falls back to static summary when API returns non-ok status", async () => {
    ...
    globalExpect(call2.summary).toMatch(/Consolidated/i);
    globalExpect(warn).toHaveBeenCalledWith(
      globalExpect.stringContaining("status 429")
    );
  });
  it("prefers consolidationModel.apiKey over hostLlmApiKey", async () => {
    ...
    globalExpect(init.headers["x-api-key"]).toBe("explicit-key");
  });
  it("caps transcript at 8000 chars before sending to LLM", async () => {
    ...
    globalExpect(promptContent.length).toBeLessThan(9e3);
  });
  it("uses llm.model as the fallback consolidation model", async () => {
    ...
    const [, init] = mockFetch.mock.calls[0];
    const body = JSON.parse(init.body);
    globalExpect(body.model).toBe("gpt-4.1-mini");
  });
});
describe("autoCapture", () => {
  it("returns captured:false when sessionId is null", async () => {
    const client = { chatAdd: vi.fn() };
    const state = createHookState();
    const result = await autoCapture(client, baseConfig, state, makeRedactor(), makeArtifacts(), "response");
    globalExpect(result.captured).toBe(false);
    globalExpect(vi.mocked(client.chatAdd)).not.toHaveBeenCalled();
  });
  ...
  it("stores assistant message via chatAdd", async () => {
    ...
    globalExpect(chatAdd).toHaveBeenCalledOnce();
    const [, role, content] = chatAdd.mock.calls[0];
    globalExpect(role).toBe("assistant");
    globalExpect(content).toBe("the answer");
  });
  it("sets state.lastAssistantResponse", async () => {
    ...
    globalExpect(state.lastAssistantResponse).toBe("trimmed response");
  });
  it("returns consolidated:false when messageCount is below threshold", async () => {
    ...
    state.messageCount = 5;
    const result = await autoCapture(client, baseConfig, state, makeRedactor(), makeArtifacts(), "response");
    globalExpect(result.captured).toBe(true);
    globalExpect(result.consolidated).toBe(false);
  });
  it("triggers consolidation and returns consolidated:true when messageCount reaches threshold", async () => {
    ...
    fullClient.chatAdd = vi.fn().mockResolvedValue(void 0);
    const state = createHookState();
    state.sessionId = "session-ac-006";
    state.lastUserMessage = "question";
    state.messageCount = 19;
    const result = await autoCapture(fullClient, baseConfig, state, makeRedactor(), makeArtifacts(), "response");
    globalExpect(result.consolidated).toBe(true);
    globalExpect(vi.mocked(fullClient.memoryStore)).toHaveBeenCalled();
  });
});
describe("prefetchMemories", () => {
  ...
  it("calls memoryRetrieve with query and topK", async () => {
    const memoryRetrieve = vi.fn().mockResolvedValue([]);
    const client = { memoryRetrieve };
    const config2 = { ...baseConfig, topK: 7 };
    await prefetchMemories(client, config2, "test query");
    globalExpect(memoryRetrieve).toHaveBeenCalledOnce();
    const [params] = memoryRetrieve.mock.calls[0];
    globalExpect(params.query).toBe("test query");
    globalExpect(params.limit).toBe(7);
  });
  it("filters out memories below searchThreshold", async () => {
    const client = {
      memoryRetrieve: vi.fn().mockResolvedValue(memories)
    };
    const config2 = { ...baseConfig, searchThreshold: 0.5 };
    const result = await prefetchMemories(client, config2, "query");
    globalExpect(result.memories).toHaveLength(1);
    globalExpect(result.memories[0].kref).toBe("kref://x/y/z?r=1");
  });
  it("returns non-empty contextInjection when memories pass threshold", async () => {
    ...
    globalExpect(result.contextInjection).not.toBe("");
    globalExpect(result.contextInjection).toContain("Test fact");
  });
  it("returns empty contextInjection when no memories pass threshold", async () => {
    const client = {
      memoryRetrieve: vi.fn().mockResolvedValue([memories[1]])
      // score 0.2 below 0.5 threshold
    };
    const config2 = { ...baseConfig, searchThreshold: 0.5 };
    const result = await prefetchMemories(client, config2, "query");
    globalExpect(result.contextInjection).toBe("");
    globalExpect(result.memories).toHaveLength(0);
  });
  it("does not call chatAdd \u2014 read-only with no state mutations", async () => {
    ...
    globalExpect(chatAdd).not.toHaveBeenCalled();
    globalExpect(state.messageCount).toBe(5);
  });
});
