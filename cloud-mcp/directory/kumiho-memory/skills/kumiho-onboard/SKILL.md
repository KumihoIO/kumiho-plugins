...
1. Inspect which Kumiho tools the host exposes. If none are available or a tool
   ...
   - Claude Code: run `/mcp`, select the Kumiho Memory server
     (`plugin:kumiho-memory:kumiho-memory`) and authenticate, or re-authenticate
     an expired login. If it is not listed, check with `/plugin` that the plugin
...
https://github.com/KumihoIO/kumiho-plugins/tree/main/codex . The local Claude Code
plugin is also named `kumiho-memory`, so keep only one of the two enabled. Do not
silently convert this OAuth connection into a local installation. User scope takes
precedence.
